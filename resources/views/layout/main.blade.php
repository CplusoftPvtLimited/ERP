...
<body onload="myFunction()">
    ...
    <nav class="side-navbar">
        ...
        <ul id="side-main-menu" class="side-menu list-unstyled">
            ...
            @endif
            <li id="retailers-list-menu" class="{{ $route == 'retailers.index' ? 'active' : '' }}"><a
                    href="{{ route('retailers.index') }}"><i class="dripicons-user-group"></i>
                    {{ trans('file.Retailers') }}</a></li>
            <li id="form-list-menu" class="{{ $route == 'form.index' ? 'active' : '' }}"><a
                    href="{{ route('form.index') }}"><i class="dripicons-list"></i>{{ trans('file.Forms') }}</a>
            </li>
                ...
                <ul id="people" class="collapse list-unstyled ">
                    ...
                    @if ($user_index_permission_active)
                        <li id="user-list-menu"><a
                                href="{{ route('user.index') }}">{{ trans('file.People List') }}</a></li>
        ...
        <header class="container-fluid">
            <nav class="navbar">
                ...
                <ul class="nav-menu list-unstyled d-flex flex-md-row align-items-md-center">
                    ...
                    <li class="nav-item">
                        <a rel="nofollow" data-toggle="tooltip" title="{{ __('Settings') }}"
                            class="nav-link dropdown-item"><i class="dripicons-gear"></i>
                        </a>
                        <ul class="right-sidebar" id="notify">
                            <li style="background-color: #F7F7F7" id="role-menu"><a
                                    href="{{ route('role.index') }}">{{ trans('file.Role Permission') }}</a></li>
                            <li style="background-color: #F7F7F7" id="general-mail-setting-menu"><a
                                    href="{{ route('setting.generalmail') }}">{{ trans('file.General Mail Setting') }}</a>
                            </li>
                            <li style="background-color: #F7F7F7" id="tax-menu"><a
                            ...
                            @foreach ($user_notifications as $noti)
                                @if ($noti->noti_type == 'registration' && $noti->read_at == null)
                                    <li style="background-color: lightgrey"><a
                                            href="{{ url('user_show', [$noti->id, $noti->data['sender_id']]) }}">{{ Str::limit($noti->data['message'], 30) }}</a>
                                    </li>
                                @elseif($noti->noti_type == 'formsubmission' && $noti->read_at == null)
                                    <li style="background-color: lightgrey"><a
                                            href="{{ url('submitted_form_show', [$noti->id, $noti->data['sender_id']]) }}">{{ Str::limit($noti->data['message'], 30) }}</a>
                                    </li>
                                @elseif($noti->noti_type == 'registration' && $noti->read_at != null)
                                    <li style="background-color: #F7F7F7"><a
                                            href="{{ url('user_show', [$noti->id, $noti->data['sender_id']]) }}">{{ Str::limit($noti->data['message'], 30) }}</a>
                                    </li>
                                @elseif($noti->noti_type == 'formsubmission' && $noti->read_at != null)
                                    <li style="background-color: #F7F7F7"><a
